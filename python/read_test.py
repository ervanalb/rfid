...
(clock_recovery, decoded_waveform, decoded_bits) = rfid.simple_decoder(samples_demod, bit_width=50)
...
for (cycle, count) in seen.items():
    ...
    print(rfid.bytes_to_binary_string(cycle))
    print(rfid.decode_manchester(rfid.bytes_to_binary_string(cycle)))
